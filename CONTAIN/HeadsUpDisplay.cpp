...
HeadsUpDisplay::HeadsUpDisplay(RESOURCES* i_resources) : resources{ i_resources }
{
	...
	healthRemainingText.setPosition(sf::Vector2f(HOR_MARGIN + (COURT_WIDTH * (1.0f / 5.0f)), 5.0f));
	BasicWeaponCharge.setPosition(sf::Vector2f(HOR_MARGIN + (COURT_WIDTH) * (3.0f / 5.0f), 5.0f));
	specialWeaponAmmo.setPosition(sf::Vector2f(HOR_MARGIN + (COURT_WIDTH * (4.0f/5.0f)), 5.0f));
